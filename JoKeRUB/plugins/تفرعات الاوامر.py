...
@l313l.ar_cmd(
    pattern="البصمات$",
    command=("البصمات ", plugin_category),
           )
async def _(event):
     if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        await event.edit(
            "**قـائـمـة الـبـصـمـات 😁\n \n ✎┊‌ اختر احدى هذه الـبـصـمـات \n\n"
            "{ `.سبايدر مان` }\n"
            "{ `.كرنج` }\n"
            "{ `.خطية` }\n"
            "{ `.قفصوني` }\n"
