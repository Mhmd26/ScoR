...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**✎┊‌ يابه گلنه لا تكرر صاحب الحساب مو موجود دبلع بلوك 😒\n- مع السلامه 🗿**"
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""**✎┊‌ هلا حبيبي {mention} \n مالك الحساب مو موجود حاليا حاول تنتظره لما يجي 😁

لديك {warns}/{totalwarns} من التحذيرات لا تكرر الرسائل حته ما تنحظر 😉 ** """
    else:
        USER_BOT_NO_WARN = f"""**✎┊‌ هلا حبيبي {mention} \n مالك الحساب مو موجود حاليا حاول تنتظره لما يجي 😁
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة حماية العقرب.txt",
