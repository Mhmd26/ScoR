...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**✎┊‌ يابه گلنه لا تكرر صاحب الحساب مو موجود 😒\n- دبلع بلوك مع السلامة 🗿**"
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""**✎┊‌ هلا حبيبي {mention} \n\n مالك الحساب مو موجود حاليا  \n حاول تنتظره لما يجي 😁

عندك {warns}/{totalwarns} من التحذيرات \nلا تكرر الرسائل حته ما تنحظر 😉 ** """
    else:
        USER_BOT_NO_WARN = f"""**✎┊‌ هلا حبيبي {mention} \n\n مالك الحساب مو موجود حاليا  \n حاول تنتظره لما يجي 😁
