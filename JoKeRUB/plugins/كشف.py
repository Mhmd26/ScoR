...
async def fetch_info(replied_user, event):
    ...
    rotbat = " مطور اساسي 𓅓 " if user_id == 7115002714 else (" عضو 𓅛 ")
    ...
    caption += f"<b> {JEP_EM} 𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞  ➟ {username}</b>\n\n"
...
async def _(event):
    ...
    if not replied_user:
        return
    catevent = await edit_or_reply(event, "**✎┊‌ جار إحضار معلومات المستخدم ⏳**")
