...
@client.on(events.NewMessage(pattern=".مراقب الروابط"))
async def command_list(event):
    await edit_or_reply(
        event, 
        "**✎┊ مراقبة المواقع | UpTime 🦂  \n\n"
        "-‌ شرح الامر : \n\n"
        "- { `.اضافة رابط` } \n اكتبه ثم اكتب بعدة رابط الموقع الذي تريد مراقبته وفحصه\n\n "
        "- { `.حذف الرابط` }\nلإزالة الرابط من المراقبه والفحص \n\n"
        "- { `.قائمة الروابط` }\nلرؤية المروابط التي تتم مراقبها \n\n"
        "- { `.تحديد المدة` }\nلتحديد مدة زيارة الرابط بالثواني من 1 الى 600 ثانية \n\n"
        "- العقرب |  𝗦𝗰𝗼𝗿𝗽𝗶𝗼 🦂**"
    )
